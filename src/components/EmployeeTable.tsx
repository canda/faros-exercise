      ...
      <table className="w-full table-fixed border-separate border-spacing-0 text-sm">
        <thead>
          <tr className="bg-slate-50 text-slate-600">
            ...
            <th className="w-[38%] border-b border-slate-200 px-3 py-2.5 text-left font-semibold">Name</th>
            <th className="w-[18%] border-b border-slate-200 px-3 py-2.5 text-left font-semibold">Tracking Status</th>
            <th className="w-[22%] border-b border-slate-200 px-3 py-2.5 text-left font-semibold">Teams</th>
            <th className="w-[14%] border-b border-slate-200 px-3 py-2.5 text-left font-semibold">
              Accounts Connected
            </th>
            <th className="w-[8%] border-b border-slate-200 px-3 py-2.5 text-right font-semibold"> </th>
          </tr>
        </thead>
        <tbody>
          {employees.map((e, idx) => {
            const rowBorder = idx === employees.length - 1 ? '' : 'border-b border-slate-100'
              ...
              <tr key={e.id} className={cn('bg-white', rowBorder)}>
                ...
                <td className="px-3 py-3 align-middle">
                  <div className="flex items-center gap-2">
                    <span className={cn('h-2.5 w-2.5 rounded-full', statusDot(e.trackingStatus))} />
                    <div className="text-slate-700">
                      <div className="font-semibold">{e.trackingStatus}</div>
                      <div className="text-xs text-slate-500">{e.trackingStatus === 'Included' ? 'Active' : 'Ignored'}</div>
                    </div>
                  </div>
                </td>
                <td className="px-3 py-3 align-middle">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {extraTeams ? (
                      <span className="text-xs font-medium text-slate-500">+{extraTeams}</span>
                    ) : null}
                  </div>
                </td>
                <td className="px-3 py-3 align-middle">
