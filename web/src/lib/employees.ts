...
export function getEmployees(): Employee[] {
  const employees = (rawEmployees as unknown as RawEmployeesData).data?.employees
  if (!employees) return []
  ...
  return employees.map((e) => {
    ...
    return {
      ...
      teams: unique((e.teams ?? []).map((t) => t.team?.name).filter(Boolean) as string[]),
...
const TITLES = ['Software Engineer', 'Senior Engineer', 'Product Engineer', 'Solutions Engineer'] as const
